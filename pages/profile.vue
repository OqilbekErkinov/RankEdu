    ...
    <div class="row g-3 align-items-stretch">
      ...
      <div class="col-lg-3">
        <div class="card border-0 shadow-sm h-100 p-3">
          <div class="d-flex align-items-center gap-3">
            <div class="ava-circle sm">{{ initials }}</div>
          ...
          <div class="mt-3">
            <div class="progress prog-thin">
              <div class="progress-bar bg-warning" :style="{ width: levelPercent + '%' }"></div>
            </div>
            <div class="d-flex justify-content-between small text-muted mt-1">
              <span>{{ remainToNext.toLocaleString() }} XP to Level {{ user.level + 1 }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="col-lg-9">
        <div class="card border-0 shadow-sm h-100 p-4 position-relative">
          <button class="btn btn-light border position-absolute top-3 end-3 rounded-3">
            Edit Profile
          </button>

          <div class="d-flex align-items-center gap-4">
            <div class="ava-circle xl">{{ initials }}</div>
            <div class="flex-grow-1">
              <h3 class="mb-1">{{ user.fullname }}</h3>
              <div class="text-muted">
                {{ user.universityFull }} <span class="mx-1">•</span> {{ user.major }}
              </div>

              <div class="d-flex flex-wrap align-items-center gap-4 mt-3">
                <div>
                  <div class="text-primary fw-semibold fs-5">Level {{ user.level }}</div>
                  <div class="small text-muted">Senior Rank</div>
                </div>
                <div>
                  <div class="fw-semibold fs-5 text-warning">{{ user.xp.toLocaleString() }} XP</div>
                  <div class="small text-muted">Total Experience</div>
                </div>
                <div>
                  <div class="fw-semibold fs-5 text-warning">#{{ user.globalRank }}</div>
...
<script setup>
const user = {
  fullname: 'Oqilbek Erkinov',
  universityShort: 'TIFT',
  universityFull: 'Tashkent International Finance and Technology University',
  major: 'Management',
  level: 12,
  xp: 7350,
  globalRank: 47
...
const badges = [
  ...
  { id: 2, icon: '⚡', title: 'Quick Learner', tier: 'silver', date: '2024-10' },
  { id: 3, icon: '🏆', title: 'Competition Winner', tier: 'gold', date: '2024-09' },
  { id: 4, icon: '🤝', title: 'Team Player', tier: 'bronze', date: '2024-09' },
  { id: 5, icon: '🤖', title: 'AI Enthusiast', tier: 'silver', date: '2024-08' },
...
.ava-circle{
  ...
  background: radial-gradient(120% 120% at 80% 20%, #bfe1ff 0%, #73b4ff 60%, #2d76ff 100%);
}
.ava-circle.sm{ width: 52px; height: 52px; font-size: 18px; }
.ava-circle.xl{ width: 96px; height: 96px; font-size: 28px; }

/* Progress bars */
.prog-fat{ height: 12px; background:#eef1f6; border-radius: 999px; }
.prog-thin{ height: 8px; background:#eef1f6; border-radius: 999px; }
.prog-fat .progress-bar, .prog-thin .progress-bar{ border-radius: 999px; }
...
.pill.gold{ background:#fff7cc; color:#8a6d00; }
.pill.silver{ background:#eef2f7; color:#4b5563; }
.pill.bronze{ background:#ffe7d2; color:#8b4513; }
...
@media (max-width: 992px){
  .ava-circle.xl{ width:84px;height:84px;font-size:26px; }
}
</style>
