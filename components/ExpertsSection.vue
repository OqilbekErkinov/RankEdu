...
<template>
  <section id="experts" class="experts py-5">
    <div class="container">
      ...
      <!-- Controls -->
      <div class="controls d-flex align-items-center justify-content-center gap-3 mt-3">
        <button class="nav btn-round" :disabled="page===0" @click="prev">‹</button>
        ...
        <button class="nav btn-round" :disabled="page===pages.length-1" @click="next">›</button>
      </div>
    </div>

    <!-- Video Modal (YouTube embed) -->
    <div class="modal fade" id="expertVideoModal" tabindex="-1" aria-hidden="true">
...
const next = () => (page.value = Math.min(pages.value.length - 1, page.value + 1))
...
.accent{
  ...
  -webkit-background-clip:text; background-clip:text; color:#0000FF;
}
...
.card-phone:focus-visible{ box-shadow: 0 0 0 4px rgba(11,95,255,.25); border-radius:22px; }

.screen{
  border-radius: 18px;
  overflow: hidden;
  border: 4px solid #fff;
  box-shadow: 0 6px 20px rgba(16,38,94,.12), inset 0 0 0 1px var(--frame);
...
.play{
  ...
  color:#fff; border:2px solid #fff; font-weight:700;
...
.controls .btn-round{
  ...
  border:1px solid #9FBDE1; background:#fff; color:#2A5E99;
  display:grid; place-items:center; font-size:18px;
}
.controls .btn-round:disabled{ opacity:.4; }

.dots{ display:flex; align-items:center; gap:8px; }
.dot{ width:8px; height:8px; border-radius:999px; background:#C9DBF3; transition:.2s; }
.dot.active{ width:36px; height:8px; background:#2A5E99; border-radius:999px; }
